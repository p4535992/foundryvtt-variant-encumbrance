...
export const VariantEncumbranceBulkImpl = {
     ...
     * @returns {{totalWeight, totalWeightToDisplay, lightMax, mediumMax, heavyMax, encumbranceTier, speedDecrease, unit, inventorySlot, minimumBulk, encumbrance, mapItemEncumbrance}}  An object describing the character's encumbrance level
     * @private
     */
        ...
        switch (effectName.toLowerCase()) {
            ...
            case ENCUMBRANCE_STATE.HEAVILY_ENCUMBERED.toLowerCase(): {
                ...
                const speedDecreased =
                    speedDecrease > 0
                        ? speedDecrease
                        : game.settings.get(CONSTANTS.MODULE_ID, "heavyWeightDecreaseBulk");
    ...
    async renderActorSheet(actorSheet, htmlElement, actorObject, actorEntityTmp, encumbranceElement, sheetClass) {
        // ===============================
...
export function calcBulkItemCollection(
    ...
    if (weightless) {
        ...
        itemWeight =
            calcItemBulk(item, ignoreCurrency, doNotIncreaseWeightByQuantityForNoAmmunition, {
                ignoreItems,
                ignoreTypes,
            }) + (getItemBulk(item) ?? 0);
    }
